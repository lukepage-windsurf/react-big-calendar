...
function isAllDayEvent(event, _ref) {
  ...
  var endsDayAfter = _dates.default.isDayAfter(eStart, eEnd);
  return (0, _accessors.accessor)(event, allDayAccessor) || !_dates.default.eq(eStart, eEnd, 'day') && !(endsDayAfter && _dates.default.timeDiffLTE(eStart, eEnd, 24, 'hours')) || _dates.default.isJustDate(eStart) && _dates.default.isJustDate(eEnd) && !endsDayAfter;
}
...
function makeEventOrAvailabilityFilter(date, startAccessor, endAccessor) {
  return function filterDayEventsOrAvailabilities(eventOrAvailability) {
    ...
    return _dates.default.inRange(date, start, (0, _accessors.accessor)(eventOrAvailability, endAccessor), 'day') && _dates.default.eq(date, start, 'day'); // ignore back end of long range events for now
  };
}