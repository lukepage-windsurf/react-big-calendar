...
function startsBefore(date, min) {
  return _dates.default.lt(_dates.default.merge(min, date), min, 'minutes');
}
function positionFromDate(date, min, total) {
  if (startsBefore(date, min)) return 0;
  var diff = _dates.default.diff(min, _dates.default.merge(min, date), 'minutes');
...
var getSlot = function getSlot(event, accessor, min, totalMin) {
  ...
  var dayStart = _dates.default.startOf(time, 'day');
  var dayEnd = _dates.default.endOf(time, 'day');
  ...
  if (isFallingBack && time.getTimezoneOffset() !== dayStart.getTimezoneOffset()) {
    time = _dates.default.add(time, daylightSavingsShift, 'minutes');
  }
  ...
  if (isEndAccessor) {
    ...
    if (_dates.default.isDayAfter(min, time)) {
      time = new Date(min);
      time.setHours(23);
      time.setMinutes(59);
    }
    ...
    if (_dates.default.isDayBefore(min, time)) {
      time = new Date(min);
    }
  }
...
function getStyledEvents(_ref7) {
  var unsortedEvents = _ref7.events,
    ...
    rightOffset = _ref7$rightOffset === void 0 ? 0 : _ref7$rightOffset;
